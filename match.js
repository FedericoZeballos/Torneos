...
class MatchManager {
  ...
  create(matchData) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Solo los administradores pueden crear partidos",
      };
    }
    ...
    if (!matchData.tournamentId || !matchData.team1Id || !matchData.team2Id) {
      return {
        success: false,
        message: "El torneo y ambos equipos son obligatorios",
      };
    }
    ...
    const tournament = db.readById("tournaments", matchData.tournamentId);
    if (!tournament) {
      return { success: false, message: "El torneo especificado no existe" };
    }

    // Verificar que los equipos existen
    const team1 = db.readById("teams", matchData.team1Id);
    const team2 = db.readById("teams", matchData.team2Id);
    if (!team1 || !team2) {
      return { success: false, message: "Uno o ambos equipos no existen" };
    }

    // Verificar que ambos equipos están registrados en el torneo
    if (
      !tournament.registeredTeams.includes(parseInt(matchData.team1Id)) ||
      !tournament.registeredTeams.includes(parseInt(matchData.team2Id))
    ) {
      return {
        success: false,
        message: "Ambos equipos deben estar registrados en el torneo",
      };
    }

    // Validar fecha programada si está presente
    if (matchData.scheduledDate) {
      const scheduledDate = new Date(matchData.scheduledDate);
    ...
    const match = {
      ...
      venue: matchData.venue ? matchData.venue.trim() : "",
      round: parseInt(matchData.round) || 1,
      status: "scheduled",
      team1Score: null,
      team2Score: null,
      winnerId: null,
      notes: matchData.notes ? matchData.notes.trim() : "",
    };
  ...
  update(id, updates) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can update matches",
      };
    }
    ...
    if (updates.team1Score !== undefined || updates.team2Score !== undefined) {
      const team1Score =
        updates.team1Score !== undefined
          ? parseInt(updates.team1Score)
          : match.team1Score;
      const team2Score =
        updates.team2Score !== undefined
          ? parseInt(updates.team2Score)
          : match.team2Score;
  ...
  getByTournament(tournamentId) {
    return db.filter(
      "matches",
      (match) => match.tournamentId === parseInt(tournamentId)
    );
  }
  ...
  recordResult(matchId, team1Score, team2Score, notes = "") {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can record match results",
      };
    }
  ...
  updateKnockoutProgression(completedMatch, winnerId) {
    ...
    if (
      !tournament ||
      !tournament.bracket ||
      tournament.bracket.type !== "knockout"
    ) {
      return;
    }
    ...
    const nextRoundMatches = db.filter(
      "matches",
      (match) =>
    ...
    if (nextMatchIndex < nextRoundMatches.length) {
      ...
      if (
        (nextMatch.team1Id || updates.team1Id) &&
        (nextMatch.team2Id || updates.team2Id)
      ) {
