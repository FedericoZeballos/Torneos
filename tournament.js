...
class TournamentManager {
  ...
  create(tournamentData) {
    ...
    if (!tournamentData.name || !tournamentData.startDate) {
      return {
        success: false,
        message: "El nombre del torneo y la fecha de inicio son obligatorios",
      };
    }
    ...
    // Validar número máximo de equipos
    const maxTeams = parseInt(tournamentData.maxTeams) || 16;
    if (maxTeams < 2 || maxTeams > 64) {
      return {
        success: false,
        message: "El número máximo de equipos debe estar entre 2 y 64",
      };
    }

    // Verificar que no exista otro torneo con el mismo nombre
    const existingTournaments = this.getAll();
    if (
      existingTournaments.find(
        (t) => t.name.toLowerCase() === tournamentData.name.toLowerCase()
      )
    ) {
      return { success: false, message: "Ya existe un torneo con ese nombre" };
    }

    const tournament = {
      name: tournamentData.name.trim(),
      description: tournamentData.description
        ? tournamentData.description.trim()
        : "",
      rules: tournamentData.rules ? tournamentData.rules.trim() : "",
  ...
  update(id, updates) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can update tournaments",
      };
    }
  ...
  delete(id) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can delete tournaments",
      };
    }
    ...
    if (success) {
      ...
      const matches = db.filter(
        "matches",
        (match) => match.tournamentId === parseInt(id)
      );
  ...
  unregisterTeam(tournamentId, teamId) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can unregister teams",
      };
    }
  ...
  generateBracket(tournamentId) {
    if (!auth.isAdmin()) {
      return {
        success: false,
        message: "Only administrators can generate brackets",
      };
    }
    ...
    if (teams.length < 2) {
      return {
        success: false,
        message: "At least 2 teams are required to generate bracket",
      };
    }
